...
public class ProjectileTrajectionMultipleLaucher : MonoBehaviour
{
    ...
    public int number = 3;
    public int origin = 2;
    public float xOffset = 2.0f; // distance between projectiles
    ...
    private void Update()
    {
        origin = Mathf.Clamp(origin, 1, number);
        List<Vector3> lauchVelocity = new List<Vector3>();

        Vector3 originV = Vector3.zero; ;
        for(int i = 1; i <= number ;i ++ )
        {
            Vector3 target = targetPlayer.position;
            float realX = target.x + (i - number) * xOffset;
            target.x = realX;

            Vector3 v = CalculateAdaptiveVelocity(target);
            lauchVelocity.Add(v);
            if(i == number)
            {
                originV = v;
        ...
        foreach(Vector3 lauchV in lauchVelocity) {
            FireProjectile(lauchV);
        }

    }

    Vector3 CalculateAdaptiveVelocity(Vector3 target)
    {
    ...
    void DrawTrajectory(Vector3 velocity)
    {
        ...
        float vY = velocity.y;
        float yDiff = targetPlayer.position.y - transform.position.y;
